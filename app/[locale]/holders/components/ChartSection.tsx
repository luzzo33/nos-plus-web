...
export function ChartSection({
  ...
  const t = useTranslations('holders.chart');
  ...
  const chartPoints = Array.isArray(chartRaw) ? chartRaw : chartRaw?.data || [];
  const chartSummary = Array.isArray(chartRaw)
    ? chartData?.chart?.summary
    : chartRaw?.summary || chartData?.chart?.summary;
  const chartMetadata = Array.isArray(chartRaw)
    ? chartData?.chart?.metadata
    : chartRaw?.metadata || chartData?.chart?.metadata;

  const minHolders = useMemo(
    () =>
      chartPoints.length ? Math.min(...chartPoints.map((p: any) => Number(p.holders) || 0)) : 0,
    [chartPoints],
  );
  const maxHolders = useMemo(
    () =>
      chartPoints.length ? Math.max(...chartPoints.map((p: any) => Number(p.holders) || 0)) : 0,
    [chartPoints],
  );
    ...
    try {
      ...
      return (
        <div className="bg-card p-3 rounded-lg border border-border shadow-xl">
          ...
          <p className={text('lg', 'lg', 'font-bold')}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-base p-4 md:p-6">
          ...
          {chartType === 'bar' ? (
            <BarChart data={chartPoints} margin={{ top: 10, right: 10, left: 0, bottom: 30 }}>
              ...
              <YAxis
                ...
                fontSize={chartFontSize}
                tick={{ fill: 'hsl(var(--muted-foreground))' }}
                domain={[minHolders, maxHolders]}
              />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="holders" fill="#3b82f6" opacity={0.8} radius={[4, 4, 0, 0]} />
              {chartSummary && (
                <ReferenceLine
            ...
            <AreaChart data={chartPoints} margin={{ top: 10, right: 10, left: 0, bottom: 30 }}>
              <defs>
                <linearGradient id="colorHoldersMain" x1="0" y1="0" x2="0" y2="1">
      ...
      {chartSummary && chartMetadata && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-3 md:gap-4 mt-4 md:mt-6 pt-4 md:pt-6 border-t border-border">
          <div>
            <p className={cn(text('3xs', '2xs'), 'text-muted-foreground')}>{t('average')}</p>
            <p className={text('xs', 'sm', 'font-bold')}>
              {(Number(chartSummary.average) || 0).toLocaleString()}
            </p>
          </div>
          <div>
            <p className={cn(text('3xs', '2xs'), 'text-muted-foreground')}>{t('maximum')}</p>
            <p className={text('xs', 'sm', 'font-bold')}>
              {(Number(chartSummary.high) || 0).toLocaleString()}
            </p>
          </div>
          <div>
            <p className={cn(text('3xs', '2xs'), 'text-muted-foreground')}>{t('minimum')}</p>
            <p className={text('xs', 'sm', 'font-bold')}>
              {(Number(chartSummary.low) || 0).toLocaleString()}
            </p>
          </div>
          <div>
            <p className={cn(text('3xs', '2xs'), 'text-muted-foreground')}>{t('dataPoints')}</p>
            <p className={text('xs', 'sm', 'font-bold')}>{Number(chartMetadata.dataPoints) || 0}</p>
          </div>
        </div>
      )}
    </motion.div>
