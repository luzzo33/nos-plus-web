...
interface HoldersHeaderProps {
  widget: any;
  mounted: boolean;
  loading?: boolean;
}

export function HoldersHeader({ widget, mounted, loading = false }: HoldersHeaderProps) {
  const { text } = useFontScale();
  const t = useTranslations('holders');
  const holdersChange24h = widget?.changes?.['24h'];
  const isPositive = holdersChange24h?.value >= 0;
