...
export function FontScaleControls() {
  ...
  const t = useTranslations('price.fontScale');
  ...
  const adjustSize = (increase: boolean) => {
    ...
    setScale(currentDevice, newValue);
  };

  const resetSize = () => {
    setScale(currentDevice, isMobile ? 110 : 100);
  };
    ...
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {isOpen && (
        <div className="absolute bottom-14 right-0 bg-card border border-border rounded-lg shadow-xl p-4 min-w-[200px]">
          <h4 className="text-sm font-medium mb-3">{t('fontSize')}</h4>
...
export { FontScaleControls as default };
