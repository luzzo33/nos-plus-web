...
export function MarketOverview({
  ...
  const t = useTranslations('price.marketOverview');
  ...
  const lastUpdatedDisplay = formatLocalFromInput(widget.lastUpdate, 'HH:mm', locale) || '—';

  const volatilityScore = stats?.volatility?.daily || 0;
  const trendStrength = stats?.trends?.strength || 0;
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            <DollarSign className="w-3.5 h-3.5 md:w-4 md:h-4 text-muted-foreground group-hover:text-primary transition-colors" />
            <span className={cn(text('2xs', 'xs'), 'text-muted-foreground')}>{t('current')}</span>
          </div>
          <p className={text('base', 'lg', 'font-bold')}>{widget.price.display}</p>
          <p className={cn(text('2xs', 'xs'), 'text-muted-foreground mt-0.5')}>
            {t('lastUpdated')} {lastUpdatedDisplay}
          </p>
        </motion.div>
            ...
            {widget.ranges[statsRange] && (
              <div className="h-1 md:h-1.5 bg-secondary rounded-full mt-2 relative">
                <div
                  ...
                  style={{
                    width: `${
                      ((widget.price.current - widget.ranges[statsRange].low) /
        ...
        >
          ...
          >
            {widget.changes[statsRange]?.value?.toFixed(2) || 'N/A'}%
          </p>
          <p className={cn(text('2xs', 'xs'), 'text-muted-foreground mt-0.5')}>
        ...
        >
          ...
          <p className={text('sm', 'base', 'font-bold')}>{widget.allTime.high.display}</p>
          <p className={cn(text('2xs', 'xs'), 'text-red-500 mt-0.5')}>
            {widget.allTime.high.percentFromATH.toFixed(1)}% {t('fromAth')}
          </p>
        </motion.div>
        ...
        >
          ...
          <p className={text('base', 'lg', 'font-bold')}>{volatilityScore.toFixed(1)}%</p>
          <p className={cn(text('2xs', 'xs'), 'text-muted-foreground mt-0.5')}>
            {volatilityScore < 5 ? t('low') : volatilityScore < 15 ? t('medium') : t('high')}
          </p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className={cn(text('2xs', 'xs'), 'text-muted-foreground')}>{t('trend')}</span>
          </div>
          <p
            className={cn(
          ...
          >
            {stats?.trends?.direction ? t(stats.trends.direction as any) : t('neutral')}
          </p>
          <p className={cn(text('2xs', 'xs'), 'text-muted-foreground mt-0.5')}>
            {t('strength')}: {trendStrength.toFixed(0)}%
          </p>
        </motion.div>
      </div>
    </div>
