...
export function AnalysisSection({
  ...
  const t = useTranslations('raydium.analysis');
  ...
  if (!stats || !widget) return null;

  const currentValue = Number(widget?.current?.[metric]?.value) || 0;
  const avg30 = Number(widget?.ranges?.['30d']?.[metric]?.average) || 0;
  const percent30 = avg30 > 0 ? (currentValue / avg30) * 100 : 100;
  const todayRank = percent30 > 110 ? 'High' : percent30 < 90 ? 'Low' : 'Moderate';

  const volatility = (() => {
    if (metric === 'liquidity') {
      return Number(stats?.metrics?.liquidity?.stability?.volatility) || 0;
    }
  ...
  const metricLabel = useTranslations('raydium.marketOverview')(metric);

  const changes = widget?.changes || {};

  const ath = widget?.ath?.[metric];
  const atl = widget?.atl?.[metric];
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div>
              ...
              <ul className={cn('space-y-1.5 md:space-y-2', text('xs', 'sm'))}>
                ...
                <li>
                  {t('currentValue')}:{' '}
                  <span className="font-bold">{widget?.current?.[metric]?.display ?? '-'}</span>
                </li>
                <li>
            ...
            <div>
              ...
              <ul className={cn('space-y-1.5 md:space-y-2', text('xs', 'sm'))}>
                {['7d', '30d'].map((p) => {
                  const v = changes?.[p]?.[metric]?.percentage;
                ...
                <li>
                  ...
                  <span className="font-bold ml-1">
                    {metric === 'liquidity'
                      ...
                      : (stats?.historical?.apr?.averageDisplay ?? '-')}
                  </span>
                </li>
              </ul>
            </div>
      ...
      <div className="card-base p-4 md:p-6">
        ...
        <div className="space-y-3 md:space-y-4">
          {Object.entries(changes).map(([period, data]: any) => {
            const v = data?.[metric]?.percentage ?? 0;
            ...
            return (
              <div key={period} className="space-y-1.5 md:space-y-2">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>
                    {t('avg')}: {widget?.ranges?.[period]?.[metric]?.averageDisplay || 'N/A'}
                  </span>
                </div>
              </div>
            );
