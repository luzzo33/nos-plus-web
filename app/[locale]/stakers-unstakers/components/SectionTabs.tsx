...
import { Eye, LineChart, Clock, BarChart3, Activity } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useFontScale } from '../hooks/useFontScale';
import { useTranslations } from 'next-intl';

type SectionId = 'overview' | 'chart' | 'table' | 'stats' | 'analysis';
...
const baseSections: Array<{ id: SectionId; icon: typeof Eye }> = [
...
export function SectionTabs({ activeSection, onSectionChange, isMobile }: SectionTabsProps) {
  ...
  const t = useTranslations('stakersUnstakers.sections');
  const sections = baseSections.map((s) => ({ ...s, label: t(s.id) }));
  ...
  return (
    <div className="flex gap-2 p-1.5 bg-secondary/50 rounded-xl w-fit backdrop-blur-sm">
