...
import type { ContractWidgetData } from '@/lib/api/types';

import { cn } from '@/lib/utils';
import { Tooltip as UiTooltip } from '@/components/ui/Tooltip';
import { useFontScale } from '../hooks/useFontScale';
import { normalizeContractStats, normalizeContractWidget } from '../utils/normalizeStakingData';

interface AnalysisSectionProps {
  stats: BalancesStatsResponse['stats'];
  widget: ContractWidgetData | undefined;
  mounted: boolean;
  metric: 'total' | 'staking' | 'unstaking';
}
...
export function AnalysisSection({
  ...
  const widget = normalizeContractWidget(rawWidget);
  const metric: SeriesKey =
    rawMetric === 'staking' ? 'stakers' : rawMetric === 'unstaking' ? 'unstakers' : 'total';
  const { text } = useFontScale();
  const t = useTranslations('stakingDetails.analysis');
  const ts = useTranslations('stakingDetails.stats');
