...
type MetricMode = 'total' | 'staking' | 'unstaking';
...
export function ChartSection({
  ...
  const t = useTranslations('stakingDetails.chart');
  const tc = useTranslations('common');

  const SERIES_META = {
    staking: { color: '#22C55E', label: t('stakers') },
    unstaking: { color: '#F97316', label: t('unstakers') },
    total: { color: '#6366F1', label: t('total') },
  } as const;
  ...
  const [visibleSeries, setVisibleSeries] = useState<{
    ...
    unstaking: boolean;
    total: boolean;
  }>({ staking: true, unstaking: true, total: false });
  ...
  const points = useMemo(() => {
    ...
    return raw
      .map((p: any) => ({
        timestamp: p.timestamp ?? p.date ?? p.time ?? null,
        total: toNumber(p.total ?? p.amountTotal ?? p.valueTotal),
        staking: toNumber(p.staking ?? p.stakers ?? p.amountStaking ?? p.valueStaking),
        unstaking: toNumber(p.unstaking ?? p.unstakers ?? p.amountUnstaking ?? p.valueUnstaking),
      }))
      .filter((entry) => entry.timestamp);
  }, [chartData]);

  const summaryRaw = (chartData as any)?.chart?.summary;
  const normalizeSummaryEntry = (entry: any) => {
    ...
    return {
      avg: coerce(record.avg ?? record.average),
      max: coerce(record.max),
      min: coerce(record.min),
    };
  };
  const summary = useMemo(() => {
    if (!summaryRaw || typeof summaryRaw !== 'object') return null;
    const record = summaryRaw as Record<string, unknown>;
    return {
      total: normalizeSummaryEntry(record.total ?? record.totalAmount ?? record.accounts),
      staking: normalizeSummaryEntry(record.staking ?? record.stakers ?? record.amountStaking),
      unstaking: normalizeSummaryEntry(
        record.unstaking ?? record.unstakers ?? record.amountUnstaking,
      ),
    };
  }, [summaryRaw]);
  const metadata = (chartData as any)?.chart;
  const calcDomain = (key: 'staking' | 'unstaking' | 'total') => {
  ...
  const domainStaking = useMemo(() => calcDomain('staking'), [points]);
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    if (visibleSeries.staking)
      rows.push({
        key: 'staking',
        label: SERIES_META.staking.label,
        color: SERIES_META.staking.color,
        value: pt.staking ?? null,
      });
    if (visibleSeries.unstaking)
      rows.push({
        key: 'unstaking',
        label: SERIES_META.unstaking.label,
        color: SERIES_META.unstaking.color,
        value: pt.unstaking ?? null,
      });
    if (visibleSeries.total)
      rows.push({
        key: 'total',
        label: SERIES_META.total.label,
        color: SERIES_META.total.color,
        value: pt.total ?? null,
      });
    return (
    ...
    <div className="card-base p-4 md:p-6">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-4 md:mb-6 gap-3 md:gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
          ...
          <div className="flex gap-1 p-1 bg-secondary rounded-lg">
            {(['staking', 'unstaking', 'total'] as const).map((key) => (
              <button
                key={key}
            ...
            <div className="space-y-3 w-full max-w-md px-8">
              {Array.from({ length: 3 }).map((_, idx) => (
                <div key={idx} className="space-y-2">
                  ...
                  <div className="relative skeleton h-6 w-full rounded">
                    <div className="absolute inset-0 skeleton-shimmer" />
                  </div>
                </div>
              ))}
            ...
            {chartType === 'line' ? (
              <LineChart data={points} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                ...
                <XAxis
                  ...
                  tickMargin={8}
                />
                {visibleSeries.staking && (
                  <YAxis
                    yAxisId="left"
                    tickFormatter={formatYAxis}
                    domain={domainStaking as any}
                    tick={{ fontSize: chartFontSize, fill: '#22C55E' }}
                  />
                )}
                {visibleSeries.unstaking && (
                  <YAxis
                    orientation={visibleSeries.staking ? 'right' : 'left'}
                    yAxisId="right"
                    tickFormatter={formatYAxis}
                    domain={domainUnstaking as any}
                    tick={{ fontSize: chartFontSize, fill: '#F97316' }}
                  />
                )}
                {!visibleSeries.staking && !visibleSeries.unstaking && visibleSeries.total && (
                  <YAxis
                    tickFormatter={formatYAxis}
                ...
                {visibleSeries.staking && (
                  <Line
                    ...
                    type="monotone"
                    dataKey="staking"
                    stroke={SERIES_META.staking.color}
                    strokeWidth={2}
                    dot={false}
                    name={SERIES_META.staking.label}
                  />
                )}
                {visibleSeries.unstaking && (
                  <Line
                    yAxisId={visibleSeries.staking ? 'right' : 'left'}
                    type="monotone"
                    dataKey="unstaking"
                    stroke={SERIES_META.unstaking.color}
                    strokeWidth={2}
                    dot={false}
                    name={SERIES_META.unstaking.label}
                  />
                )}
                {visibleSeries.total && (
                  <Line
                    yAxisId={
                      visibleSeries.staking ? 'left' : visibleSeries.unstaking ? 'right' : undefined
                    }
                    type="monotone"
                    dataKey="total"
              ...
              <AreaChart data={points} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                <defs>
                  ...
                  <linearGradient id="areaStaking" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#22C55E" stopOpacity={0.25} />
                    <stop offset="95%" stopColor="#22C55E" stopOpacity={0} />
                  </linearGradient>
                  <linearGradient id="areaUnstaking" x1="0" y1="0" x2="0" y2="1">
      ...
      {summary && (
        <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-border space-y-4 md:space-y-6">
          {(['staking', 'unstaking', 'total'] as const)
            ...
            .map((k, idx) => {
              ...
              return (
                <div key={k}>
                  <p
                    className={cn(text('2xs', 'xs'), 'mb-1 font-medium')}
