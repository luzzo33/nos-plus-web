...
import type { VolumeTableResponse } from '@/lib/api/types';
import { formatLocalFromInput } from '@/lib/time';

interface TableSectionProps {
  tableData?: VolumeTableResponse;
...
export function TableSection({
  ...
  const t = useTranslations('volume.table');
  ...
  const tableColumns: VolumeTableResponse['table']['columns'] = tableData?.table?.columns ?? [];
  const tableRows: VolumeTableResponse['table']['rows'] = tableData?.table?.rows ?? [];
  ...
  const renderCell = (
    key: string,
    row: VolumeTableResponse['table']['rows'][number],
  ): ReactNode => {
    switch (key) {
      case 'timestamp':
        return mounted
          ...
          : row.timestamp;
      case 'volume':
        return row.display?.volume ?? formatVolume(row.volume ?? 0);
      case 'change': {
      ...
      case 'high':
        return row.display?.high ?? formatVolume(row.high ?? 0);
      case 'low':
        return row.display?.low ?? formatVolume(row.low ?? 0);
      case 'dataPoints':
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-secondary/50">
              {tableColumns.map((column: { key: string; label: string; sortable: boolean }) => (
                <th
                  key={column.key}
                  className={cn(
