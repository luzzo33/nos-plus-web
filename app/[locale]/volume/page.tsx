...
export default function VolumePage() {
  ...
  const t = useTranslations('volume.toast');
  ...
  const {
    ...
    queryKey: ['volume-chart-data', chartParams],
    queryFn: () => apiClient.getVolumeChartData(chartParams),
  ...
  const {
    ...
    queryKey: ['volume-stats', statsRange],
    queryFn: () => apiClient.getVolumeStats({ range: statsRange }),
  });
  ...
  const {
    ...
    queryKey: ['volume-table', tableParams],
    queryFn: () => apiClient.getVolumeTableData(tableParams),
  });
  ...
  const handleRefresh = async () => {
    ...
    setTimeout(() => {
      ...
      addToast({
        ...
        description: t('allVolumeDataUpdated'),
      });
    }, 1000);
  };
  ...
  const downloadData = async (type: 'chart' | 'table', format: 'csv' | 'json') => {
    ...
    if (type === 'chart' && chartData?.chart?.data?.data) {
      ...
      filename = `nos-volume-chart-${selectedRange}`;
    } else if (type === 'table' && tableData?.table?.rows) {
      data = tableData.table.rows;
      filename = `nos-volume-history-${tableTimeframe}`;
    }
  ...
  return (
    <FontScaleProvider>
      <div className="space-y-6 pb-12">
        {/* Header */}
        <VolumeHeader widget={widget} mounted={mounted} loading={widgetIsLoading} />
