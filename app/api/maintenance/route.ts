...
export async function POST(req: Request) {
  try {
    ...
    let ok = false;
    if (PASS_HASH) {
      ok = sha256Hex(password) === PASS_HASH;
    } else if (PASS_PLAIN) {
      ok = password === PASS_PLAIN;
    } else {
      ok = password === DEV_FALLBACK;
    }
    if (!ok) {
      return NextResponse.json({ success: false, error: 'DENIED' }, { status: 401 });
    }
    const tag = sha256Hex('m:' + Date.now()).slice(0, 16);
    const value = `v1:${subtleHash(tag + ':' + Date.now())}`;
    const res = NextResponse.json({ success: true, token: value });
    res.cookies.set('m_access', value, {
      httpOnly: true,
      sameSite: 'lax',
      maxAge: 60 * 60 * 12,
