...
export async function fetchDashboardSnapshot(opts?: {
  ...
  const cacheHasDca = !!(
    bootstrapCache &&
    Array.isArray((bootstrapCache as any).dca?.rows) &&
    (bootstrapCache as any).dca.rows.length > 0
  );
...
export function MonitorDashboardProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const needsRefresh =
      !snapshot ||
      !snapshot.dca ||
      !Array.isArray((snapshot as any).dca?.rows) ||
      ((snapshot as any).dca?.rows?.length ?? 0) === 0;
